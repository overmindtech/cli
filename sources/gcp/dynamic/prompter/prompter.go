...
- Understand what the cloud source is and what it does. Add this short description as a comment for the registarableAdapter variable.
...
- For terraform mappings, we should find the official terraform reference. If the terraform resource has an item that can be used in the get function, then this is our mapping. Be careful that if it is a regional or zonal adapter, the region and zone are part of the scope, so they can't be passed in the GET method. In these cases, we might consider using SEARCH with an attribute that has the fully qualified name including the project id like: "projects/...". If adapter does not support search, we should create a no-op one just for terraform mapping. An example can be found in the "sources/gcp/dynamic/adapters/orgpolicy-policy.go" file. In majority, if it is a regional or zonal adapter, we will go for SEARCH with the "id" attribute.
...
- Validate the source: if terraform mappings supports SEARCH, then the adapter MUST also support SEARCH.`
